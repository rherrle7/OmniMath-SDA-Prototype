...
# Initialize session state to track where the user is in the logic loop
...
# --- State 3: Phase 1 (The Initial Distractor Choice) ---
    ...
    st.button("I added 9 to both sides", on_click=set_stage, args=('mirror',))
    st.button("I subtracted 9 from both sides", on_click=set_stage, args=('trap',))
    st.button("I guessed", on_click=set_stage, args=('guessed',))

# --- State 4: The Mismatch Trap ---
    ...
    st.button("I actually added 9", on_click=set_stage, args=('mirror',))
    st.button("I guessed", on_click=set_stage, args=('guessed',))

# --- State 5: The Socratic Mirror (Grace Period) ---
...
# --- State 6: Teacher Escalation (Circuit Breaker) ---
elif st.session_state.stage == 'escalate':
    st.error("⚠️ **Concept Alert:** Please raise your hand and conference with your teacher.")
    # Notice there is no reset button here. The system locks until the teacher intervenes.
...
# --- State 8: Placeholder for other distractors ---
elif st.session_state.stage in ['dist_16', 'dist_9']:
    st.warning("You selected an incorrect answer. (Telemetry for this distractor will be mapped in the next update).")
    st.button("Reset", on_click=reset)
